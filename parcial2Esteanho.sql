...
/* 4) */
SELECT FIBRA_OPTICA_ID, FECHA_ASIGNACION - FECHA_INI FROM FALLA WHERE FECHA_ASIGNACION IS NOT NULL order by (FECHA_ASIGNACION - FECHA_INI) DESC; /* Da ladilla :v */

/* 5) */

SELECT c.DESCRIPCION, f.TIEMPO_PROMEDIO
FROM CAUSA c
JOIN (
  SELECT CAUSA_ID, COUNT(*) AS CANTIDAD_CAUSA,
         SUM(FECHA_FIN - FECHA_INI) / COUNT(*) AS TIEMPO_PROMEDIO
  FROM FALLA
  WHERE FECHA_FIN IS NOT NULL
  GROUP BY CAUSA_ID
) f ON c.ID = f.CAUSA_ID;
...
FROM RECURSO r
JOIN (
  SELECT * FROM RECURSO_FALLA rf
  JOIN FALLA f ON (rf.FALLA_FECHA_INI = f.FECHA_INI AND rf.FALLA_FIBRA_OPTICA_ID = f.FIBRA_OPTICA_ID)
  WHERE f.FECHA_ASIGNACION IS NOT NULL AND ((SYSDATE - f.FECHA_ASIGNACION) <= 45)
) a ON r.ID = a.RECURSO_RCO_ID;

SELECT r.ID, r.NOMBRE, count(*)
FROM RECURSO r
JOIN (
  SELECT * FROM RECURSO_FALLA rf
  JOIN FALLA f ON (rf.FALLA_FECHA_INI = f.FECHA_INI AND rf.FALLA_FIBRA_OPTICA_ID = f.FIBRA_OPTICA_ID)
